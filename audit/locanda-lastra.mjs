/* La lastra della Locanda del Castello, fotografata per intero.
   uso: node audit/locanda-lastra.mjs */
...
for (const [nome, vp] of [['telefono', { width: 390, height: 844 }],
  ...
  await p.evaluate(() => Promise.all([...document.querySelectorAll('.locanda__marchio img')]
    .map(i => i.decode().catch(() => {}))));
  await p.waitForTimeout(300);
  await (await p.$('.locanda__slab')).screenshot({
    path: `audit/v2shots/locanda-${nome}.jpg`, type: 'jpeg', quality: 86 });
  const m = await p.evaluate(() => {
    const s = document.querySelector('.locanda__slab').getBoundingClientRect();
    const d = document.querySelector('.locanda__marchio img').getBoundingClientRect();
    return { lastra: [Math.round(s.width), Math.round(s.height)],
             marchio: [Math.round(d.width), Math.round(d.height)],
